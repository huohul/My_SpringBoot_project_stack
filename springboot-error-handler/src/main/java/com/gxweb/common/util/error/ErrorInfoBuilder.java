...
import com.gxweb.entity.ErrorInfo;
...
public class ErrorInfoBuilder implements HandlerExceptionResolver, Ordered {
    ...
    public ErrorInfo getErrorInfo(HttpServletRequest request, Throwable error) {
        ...
        errorInfo.setError(error.toString());
                                //获取通信状态(HttpStatus)
        errorInfo.setStatusCode(getHttpStatus(request).value());  //状态码
                                 //获取通信状态(HttpStatus)
        errorInfo.setReasonPhrase(getHttpStatus(request).getReasonPhrase()); //状态码 原因短语
                                //获取堆栈轨迹(StackTrace) |判断是否包含堆栈轨迹.(isIncludeStackTrace)
